...
class DerivativeHarmonicSpace():
    ...
    EXAMPLES::
        ...
        sage: DerivativeHarmonicSpace(QQ, 4)
        <__main__.DerivativeHarmonicSpace instance at ...>
    """
...
class DerivativeVandermondeSpaceWithInert(UniqueRepresentation):
    """
    The space spanned by a generalized version of the Vandermonde determinant 
    containing inert variables with respect to the derivation and its derivatives.
    
    Let define $\Delta_{\mu} := \det( x_i^a \theta_i^b )_{1 \leq i \leq n \atop (a,b) \in \mu}$
    where $x = (x_1, x_2, \dots, x_n)$ is a set of regular variables and 
    $\theta = (\theta_1, \theta_2, \dots, \theta_n)$ is a set of inert variables, 
    that is to say variables that can't appear in the derivation.
    
    Let $W$ be the smallest submodule generated by the generalized Vandermonde determinant
    $\Delta_{\mu}$ closed under partial derivatives in the $x$ variables.
    ...
    def degree_vandermonde(self):
        ...
        w.r.t the `x` variables.
        The `theta` variables are considered to be of degree 0.
    ...
    def basis(self, verbose=False):
        ...
        for nu in Partitions(n):
            ...
            #operators[(D((-1,)),nu)] += [make_deriv_comp_young2(X[0], 2, nu)]
            #operators[(D((-2,)),nu)] = [make_deriv_comp_young2(X[0], 3, nu)]
