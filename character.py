...
def character_with_inert(mu,parallel=True): 
    ...
    return into_schur(DPRI,character_plain(mu,parallel=parallel))
...
def character_by_isotypic(P,nu,basis):
    ...
    """
    charac = 0
    S = Subspace(generators={d:B for d,B in basis},operators=polarization_operators_by_degree())
    for b in S.basis().values():
        charac += sum(P.multipower(P.multidegree(p),P.polynomial_ring().gens()) for p in b)
    return charac

def character_plain(P,mu,parallel=False):
    """
