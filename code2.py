...
class DiagonalPolynomialRingInert(DiagonalPolynomialRing):
    ...
    def polarizators_by_degree(self):
        """
            ...
            EXAMPLES::
                sage: DP.polarizators_by_degree(diff=False,pol=True)
    ...
    @cached_method
    def isotypic_basis(self,mu,use_antisymmetries=True,verbose=True):
        """
            ...
            EXAMPLES::
                sage: DP = DiagonalPolynomialRingInert(QQ,3,3)
                sage: DP.isotypic_basis(Partition([2,1]),verbose=False)
                {(0, [2, 1]): [-3*x20], (1, [1, 1, 1]): [x00*x21]}
                sage: DP.isotypic_basis(Partition([3]),verbose=False)
