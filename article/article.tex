...
\begin{document}
	...
	The character of $\mathcal{E}_{\lambda}^{(r)}$ stabilizes for $r \geq n$ (see \cite{Bergeron2013} or \cite{Bergeron2009}). Therefore, when the precision is not necessary, we will drop the $(r)$ and simply write $\mathcal{E}_{\lambda}$ for $\mathcal{E}_{\lambda}^{(r)}$ with $r \geq n$. \\
	...
	\begin{example} \label{H_3} Compute $\harmonics_3$ with one row of variables $\bm{x} = (x_1, x_2, x_3)$. \pauline{Je n'ai trouvé comment avoir les output en latex avec sauts de ligne automatiques.}
		\begin{sagesilent}
			load("../subspace.py")
		\end{sagesilent}
