...
\nicolas{Ajouter des exemples au fur et à mesure}
	...
	\begin{figure}[H]
		...
		\draw node{$\begin{pmatrix}
			...
			\end{pmatrix}$ } (0,0);
		\draw (-2.4,.2)+(60:.25)
		[xscale=.5,yscale=3.2,->,rotate=80] arc(-40:290:.25);
		\node at (-3,0) {{\small$\GLr$}};
		\draw (-.5,-1.5)+(60:.25)
		[xscale=5.5,yscale=.6,->,rotate=-210] arc(-40:290:.25);
		\node at (0,-2) {{\small$\Sn$}};
		\end{tikzpicture}
	...
	Like for $\harmonics_n^{(r)}$, the $\GLr \times \Sn$-character of $\mathcal{E}_\lambda^{(r)}$ is not easy to compute. \\
	
	\subsection{Further work} 
	
	\pauline{Change title}
