...
\begin{document}
	...
	This is a potentially huge calculation ($\harmonics_6^{(5)}$ is of dimension $5.8 \cdot10^5$), involving polynomials of degree up to $15$ in $30$ variables with thousands of terms. So we had to combine several strategies that each reduced the time and/or memory complexity by one or two orders of magnitude. As much as possible, the code was written to be modular and reusable.
...
\nicolas{Ajouter des exemples au fur et à mesure}
    ...
    In order to study those spaces, we will use representation theory and characters.  Intuitively, through representation theory, the purpose is to cut modules into irreducible submodules according to the group action. Those submodules are then represented by irreducible characters that encodes the group action on each submodule. For $\GLr$ and $\Sn$, irreducible characters can be indexed by partitions and encoded by Schur functions. Knowing the irreducible characters gives informations of the studied module such as the dimension. 
    
    We will not enter into details on representation theory here but for more on this topic see \cite{FultonHarris1991}. Also, \cite{Sagan} is a classical reference for representation theory of the symmetric group and see \cite{Green2007} for more on the polynomial representations of the general linear group. \\ 
	...
	Solving those differential equations would not be tractable either. So instead we used operators, partial derivatives and polarization operators defined in the previous section. \\
	...
	\begin{figure}[H]
		...
		\draw (-3,-.85) -- (-1.5,-.85);
		\end{tikzpicture}
		\caption{Simultaneous actions of $\GLr$ and $\Sn$ on $X$ with inert variables.}
	\end{figure}
	...
	Like for $\harmonics_n^{(r)}$, the $\GLr \times \Sn$-character of $\mathcal{E}_\lambda^{(r)}$ is not easy to compute but hopefully most of the strategies we used to compute the character of $\harmonics_{n}^{(r)}$ work for $\mathcal{E}_\lambda^{(r)}$. 
	
	Therefore, we, once again, use polarization to compute $\mathcal{E}_\lambda^{(r)}$ from $\mathcal{E}_\lambda^{(1)}$, exploit the multigrading. We use the same isotypic projector to obtain the isotypic components, then we can still exploit the columns antisymmetries. 
%	\begin{example} ~
%		
...
%	\end{example}
	Finally, $\mathcal{E}_\lambda^{(r)}$ is still symmetric with respect to a permutation of the rows of $X$ containing classical variables so we exploit row symmetries in this case too.\\
	
	\begin{example} In the following example, we use different strategies to compute $\mathcal{E}_{2,1}^{(2)}$. 
		
		First, we compute $\mathcal{E}_{2,1}^{(1)}$, for one row of variables exploiting multidegree. 
		\begin{sagecommandline}
		...
		\begin{sagecommandline}
			...
			sage: E_21
		\end{sagecommandline}
		Then, we use polarization operators to introduce a second set of variables, and thus obtain $\mathcal{E}_{2,1}^{(2)}$, still with multidegree. We also exploit row symmetries. 
		\begin{sagecommandline}
		...
		\end{sagecommandline}
		And we easily obtain the character for $\mathcal{E}_{2,1}$. 
	 	\begin{sagecommandline}
		 	sage: character(E_21, row_symmetry="permutation")
		\end{sagecommandline}
	
	So the character of $\mathcal{E}_{2,1}^{(r)}$ for $r \geq 2$ is $$\mathcal{E}_{2,1}^{(r)} = 1 \otimes s_{2,1} + s_{1} \otimes s_{1,1,1} .$$
	\end{example}
	
	\subsection{Further work} 
	
	Talk about non partition diagram : compositions and diagram with hole(s) (quotient). 
