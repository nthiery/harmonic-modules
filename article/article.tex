...
\begin{document}
	...
	The development of this package started at the occasion of a collaboration with François Bergeron to study the character of the space $\harmonics_n^{(r)}$ of diagonal harmonic polynomials on $r$ sets of $n$ variables, as a graded bi-$\GLr \times \Sn$-module. It was then extended to compute other character on polynomials spaces. The main achievement is a calculation of this character of the diagonal harmonic polynomials for $n=6$ \pauline{+ some cases for $n=7$} and $r=5$ (and therefore any $r$) in 45 minutes (and ~15Gb), which led to conjectures on its general value.
	...
	We first need to compute in a quotient by an ideal, $R / \mathcal{J}_n$ where $\mathcal{J}_n$ is the ideal of diagonal symmetric polynomials on $r$ sets of $n$ variables. \pauline{$\langle Sym(X)^+ \rangle$ ?}
	...
	As said before, the character of $\GLr$ is given by the multidegree of the homogeneous components. In other words, for a given isotypic component of type $\mu$, the $\GLr$-character associated to this component is given by $\sum_{\mathcal{E}} q^{d(\mathcal{E})}$ where the sum is over all homogeneous $\mathcal{E}$ component of type $\mu$, $d(\mathcal{E})$ is the multidegree of the component $\mathcal{E}$ and $q^d = q_1^{d_1}q_2^{d_2}\dots$. \pauline{Find better notation.} 
	
	When exploiting the row symmetries, since we don't compute only the homogeneous components of decreasing multidegree, we can not compute the character of $\GLr$ using such a summation. However, we know that if we have a component of multidegree $d=(d_1, d_2, \dots, d_k)$ , this correspond to a term $\sum_{\sigma \in \Sym{k}} q^{\sigma(d)} = m_d$ in the previous sum, thus we use monomials symmetric functions instead of summations to recover the character of $\GLr$ in each isotypic component.
	\pauline{Add time computation comparison (with and without optimisations) ?} \\
	
	For now we don't avoid to compute unnecessary homogeneous component since all we do is straighten the polynomials if they don't have decreasing multidegree. It would be even better if we could avoid completely computation of those polynomials. It is not possible for now because we don't know yet which operators should not been applied to avoid those polynomials. 
