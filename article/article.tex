...
\nicolas{Ajouter des exemples au fur et à mesure}
	...
	The space of harmonic polynomials in $n$ variables, denoted $\harmonics_{n}$, is the joint kernel of the differential operators $D_k := \partial x_1^k + \cdots + \partial x_n^k$. It is also a realization of the coinvariants of the symmetric group: $R\, /\, \langle Sym(X)^+ \rangle$ and a graded regular representation of $\Sn$. 
	
	\begin{example}
	Let consider the case of two variables $x_1$ and $x_2$. 
	In this case, polynomials with only a constant term are harmonic. Moreover, $x_1-x_2$ is harmonic but $x_1^2-x_2^2$ is not. In fact, linear combinations of constants and $x_1-x_2$ are the only harmonic polynomials in two variables so $\harmonics_2 = \QQ \{ 1, x_1-x_2 \}$. 
	\end{example}
	
	\begin{example}
		The Vandermonde determinant $V_n:= \prod_{i<j}(x_i-x_j) = \det(x_i^a)_{1 \leq i \leq n \atop 0 \leq a \leq n-1} $ is harmonic. Indeed, it is well-known that the Vandermonde determinant is the antisymmetric polynomial in $n$ variables with the smallest degree. Therefore, since $\sum_{i=1}^{n} \partial x_i^k V_n$ is still antisymmetric but of a lower degree, it has to be 0. 
	\end{example}
		
	Since derivatives on polynomials commute, if $f$ is harmonic then $\partial x_i f$ is also harmonic for all $x_i \in X$. Using that, one can define $\harmonics_{n}$ directly with  partial derivatives instead of seeing them as solutions of a system of partial differential equations as stated in the following theorem. See \cite{Haiman2002} or \cite{Bergeron2009} for more details. 
	...
	$$D_{(\alpha_1, \dots, \alpha_r)} := \partial x_{1,1}^{\alpha_1}\dots\partial x_{r,1}^{\alpha_r} + \dots + \partial x_{1,n}^{\alpha_1}\dots\partial x_{r,n}^{\alpha_r}$$
	...
	\begin{definition}
		The space of \emph{diagonal harmonic polynomials} is the joint kernel of the differential operators $(D_\alpha)_\alpha$. 
	\end{definition}
	
	Note that the space of diagonal harmonic polynomials contains $\harmonics_{n}^{(1)} = \harmonics_{n}$.
	As for harmonic polynomials, we would like an alternative and more direct way to obtain this space directly using operators. 
	...
	\begin{conjecture} \label{DiagHarm}
		Denote $\harmonics_n^{(r)}$ the smallest subspace containing the Vandermonde determinant and closed by partial derivatives and polarisation operators : $\mathcal{P}_{\mathcal{D}}(\QQ\{V_n\})$, where $\mathcal{P}_{\mathcal{D}}$ denote the closure by partial derivatives and polarisation. Then, the space of diagonal harmonic polynomials and $\harmonics_n^{(r)}$ are the same space.
	\end{conjecture}
	
	Conjecture \ref{DiagHarm} has been proven for two sets of variables (see for example \cite{Bergeron2009}), but it has not been proven yet for more than two sets of variables. From now, we work only with $\harmonics_{n}^{(r)}$ as defined in this conjecture. \\
	
	Let now construct a similar subspace of $R'$ using inert variables. Let $\lambda$ be a partition of $n$ and note $(a,b)$ the Cartesian coordinates of the South West corner of its cells. Then we define the determinant $V_{\lambda}$ associated to $\lambda$ by $$V_{\lambda} := \det (x_i^a \theta_i^b)_{1 \leq i \leq n \atop (a,b) \in \lambda} .$$
	...
	The space $\harmonics_n^{(r)}$ is thus a special case of $\Elambdar{\lambda}{r}$ for $\lambda = (n)$.
	
	The character of $\Elambdar{\lambda}{r}$ stabilizes for $r \geq n$ (see \cite{Bergeron2009} or \cite{Bergeron2013}). This means that $r=n-1$ sets of variables are necessary to obtain all the terms of the character and adding another set or more of variables won't change the result. Therefore, when the precision is not necessary, we drop the exponent and simply write $\Elambda{\lambda}$ for $\Elambdar{\lambda}{r}$ with $r \geq n$. 
	This property also gives us an upper bound for the number of sets of variables needed in the computation. \\
	...
	\subsection{Computing in $\Elambdar{n}{r}$ using polarization}
	...
	Solving those differential equations would not be tractable either. So instead, based on conjecture \ref{DiagHarm}, we use partial derivatives and polarization operators defined in the previous section. \pauline{Reformulation nécessaire, pas la preuve que ce sont les mêmes espaces alors raccourcis pas très rigoureux présenté ainsi.}\\
	...
	\begin{example} \label{H_3} Compute $\harmonics_3$ with one row of variables $\bm{x} = (x_1, x_2, x_3)$. \pauline{I haven't find out a way to have latex output with automatic breaklines.}
		...
		\begin{sagecommandline}
			...
			sage: H3 = Subspace([M.determinant()], derivatives)
			sage: H3.basis()
			sage: H3.dimension()
		\end{sagecommandline}
	
	With two rows of variables. 
		\begin{sagesilent}
			load("../character.py")
		\end{sagesilent}
		\begin{sagecommandline}
			sage: v = vandermonde(3,2)
			sage: derivatives = [attrcall("derivative", x) for i in range(2) for x in v.parent().multivar_pol_ring_variables()[i] ]
			sage: polarizators = [attrcall("polarization", i1=i1, i2=i2, d=d) for d in range(1,4) for i1 in range(0, 2) for i2 in range(0, 2) if (i1!=i2)]
			sage: H3 = Subspace([v], derivatives+polarizators)
			sage: H3.dimension() 	
		\end{sagecommandline}
	\pauline{Problème dimension, j'ai trouvé l'élément intru mais je sais pas pourquoi il apparaît}
	\end{example} 
	...
	At this stage, we are interested in reducing the number of operations computed by our \texttt{Subspace} utility. One way to do that would be to need less operators to obtain the same basis. This can be done by considering the quasi-commutativity of the partial derivatives and the polarisation operators. 
	
	\begin{prop}\label{prop_commutativite}
		For all subspace $\mathcal{W}$ of $R'$,
		$$\mathcal{P}_\mathcal{D}(\mathcal{W}) = \mathcal{D} \circ \mathcal{P}(\mathcal{W}) = \mathcal{P} \circ \mathcal{D}(\mathcal{W}),$$
		where $\mathcal{P}$ is the closure by polarisation operators.
	\end{prop}

	\begin{proof}
		If we apply a partial derivative and a polarisation operator with no variable in common on an element of $R'$, applying them on any order does not change the result. But let now look at what happen if those operators do have variables on common. 
		
		\begin{equation}\label{polpartial}
		\partial y_i^l P^k_{\x,\y}f - P^k_{\x,\y} \partial y_i ^l f 
		= \partial y_i ^l \sum_{j=1}^{n} y_j \partial x_j ^kf - \sum_{j=1}^{n} y_j \partial x_j ^k \partial y_i ^l f
		\end{equation}
		
		For $i=j$, we temporarily removed indices and we obtain, 
		$\partial y^ly \partial x^k f - y \partial x^k \partial y^l = l\partial y^{l-1} \partial x^kf + y\partial y^{l} \partial x^kf - y\partial x^k \partial y^{l} f = l\partial y^{l-1} \partial x^kf$.
		Using this result in equation \ref{polpartial}, we have 
		\begin{equation}\label{polpartiali}
		\partial y_i^l P^k_{\x,\y}f - P^k_{\x,\y} \partial y_i^l f  =  l\sum_{i=1}^{n} \partial y_i^{l-1} \partial x_i^k f.
		\end{equation}
		
		Thus we obtain a sum of composed partial derivatives. Since the spaces we study are closed by derivation, the commutation of partial derivatives and polarisation operators gives operators we are already using to build the subspace. Therefore, we don't obtain new linear independent elements by commuting those operators.  
	\end{proof}

	This phenomena is due to the commutativity of the action of $\GLr$ represented by the polarisation operators and the action of $\Sn$ of $V_\gamma$  and its partial derivatives. 
	
	In our case, $\mathcal{W} = \QQ\{V_\lambda\}$ and we want to compute $\Elambdar{\lambda}{r} = \mathcal{P}_{\mathcal{D}}(\QQ\{V_\lambda\})$. Proposition \ref{prop_commutativite} implies that we can first compute a basis $\mathbb{B}_\lambda^{(1)}$ of the space $\Elambdar{\lambda}{1} = \mathcal{D}(\QQ\{V_\lambda\})$. Then, we can obtain a basis of $\Elambdar{\lambda}{r}$ from $\mathcal{P}(\mathcal{B}_\gamma^{(1)})$. 
	
	This way, we use only the partial derivatives on the first set of variables of $X'$ and we reduce the amount of compositions of operators to compute.  
	...
	\begin{example}
		...
		\begin{sagecommandline}
			...
			sage: derivatives
			sage: E3 = Subspace({v.multidegree() : [v]}, derivatives)
			sage: E3.basis()
			sage: E3.dimension()
			sage: polarizators = polarization_operators(2)
			sage: polarizators
			sage: E3 = PolarizedSpace(E3, polarizators)
			sage: E3.dimension()
			sage: E3.dimensions()
		\end{sagecommandline}
	\end{example}
	...
	\begin{example}
		...
		\begin{sagecommandline}
			sage: p = list(H_3[Partition([2,1])].basis().values())[0][0]
			sage: p
			sage: p.parent()
		\end{sagecommandline}
